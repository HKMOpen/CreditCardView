...
public class CreditCardView extends FrameLayout {
    ...
    private static final int FRONT_QR_CODE = R.id.qr_code_view;

    private int card_provider_logo = 0;
    ...
    public void setCardNumber(@Nullable String rawCardNumber) {
        ...
        TextView djTex = (TextView) findViewById(TEXTVIEW_CARD_NUMBER_ID);
        djTex.setText(cardNumber);
        if (this.mRawCardNumber.length() > 0 && CardType.BITCOIN.isMatched(this.mRawCardNumber)) {
            djTex.setTextSize(10f);
        }
        if (this.mRawCardNumber.length() == 5) {
            revealCardAnimation();
        } else {
            paintCard();
        }
    ...
    public void paintCard() {
        ...
        ImageView image_view = (ImageView) findViewById(FRONT_QR_CODE);
        ...
        card_provider_logo = card.getResLogoId();
        ...
        if (CardType.BITCOIN.isMatched(this.mRawCardNumber)) {
            Bitmap myBitmap = QRCode.from(this.mRawCardNumber).bitmap();
    ...
    public CardSelector selectCard() {
        ...
        return CardSelector.selectCard(mRawCardNumber).getCardFace();
    }
